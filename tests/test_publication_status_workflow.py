...
def test_publication_status_workflow_reports_non_blocking_status() -> None:
    ...
    assert (
        '--status-report "$RUNNER_TEMP/publication-status-artifact/reports/status/status_report.json"'
        in workflow
    )
    assert (
        '--publication-evidence "$RUNNER_TEMP/publication-status-artifact/publication_evidence/publication_evidence.json"'
        in workflow
    )
    assert (
        '--blocker-report "$RUNNER_TEMP/publication-status-artifact/blockers/blocker_report.json"'
        in workflow
    )

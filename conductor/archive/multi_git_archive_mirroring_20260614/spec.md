...
## Overview
This track implements multi-git repository mirroring and backup publishing strategies for the `hathi-nz` debates fetcher and dataset pipeline to guarantee durability and prevent censorship or single-point-of-failure repository/dataset takedowns.
...
- Track-local strategy document records which mirroring actions are local-only and which require explicit external-account approval.
